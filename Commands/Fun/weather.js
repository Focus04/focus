  ...
  async execute (message, args, prefix) {
    if (!args[0]) {
      let msg = await message.channel.send(
        `Proper command usage: ${prefix}weather [location]`
      )
    ...
    let response = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${location}&units=metric&appid=${process.env.weatherid}`
    )
    let data = await response.json()
    if (data.message === 'city not found') {
      let msg = await message.channel.send(
        `Couldn't find any weather results for ${location}.`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    ...
    let weatherembed = new Discord.MessageEmbed()
      ...
      .addFields(
        ...
        {
          name: 'Pressure',
          value: `${Math.floor(data.main.pressure / 1.3)} mm Hg`
        },
        {
          name: 'Wind',
          value: `${Math.floor(data.wind.speed * 3.6)} km/h, ${data.wind.deg} °`
        }
      )
