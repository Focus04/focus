...
module.exports = {
  ...
  async execute (message, args, prefix) {
    ...
    if (!args[1]) {
      let msg = await message.channel.send(
        `Proper command usage: ${prefix}takerole @[member] [role]`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    ...
    let role = member.roles.cache.find(role =>
      role.name.toLowerCase().startsWith(rolename)
    )
    if (!role) {
      let msg = await message.channel.send(
        `${member.user.username} doesn't have any roles named ${rolename}`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    ...
    if (role.position >= bothighestrole) {
      let msg = await message.channel.send(
        'My roles must be higher than the role that you want to take!'
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    ...
    if (role.position >= highestrole) {
      let msg = await message.channel.send(
        'Your roles must be higher than the role that you want to take.'
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
