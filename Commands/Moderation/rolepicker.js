...
module.exports = {
  name: 'rolepicker',
  description:
    'Creates a menu that automatically assigns roles to users that react to it.',
  usage:
    'rolepicker @`role` emoji @`role` emoji @`role` emoji etc. (maximum 25)',
  guildOnly: true,
  async execute (message, args, prefix) {
    ...
    args.forEach(async arg => {
      ...
      if (args.indexOf(arg) % 2 == 1 && !arg.startsWith('<@&')) {
        if (arg.startsWith('<:')) {
          ...
          if (!customemoji) {
            let msg = await message.channel.send(
              `Error at emoji ${arg}. Make sure that this emoji shares a mutual server with me!`
            )
            return msg.delete({ timeout: 10000 })
          }
    ...
    if (!message.guild.me.hasPermission('MANAGE_ROLES')) {
      let msg = await message.channel.send(
        'I require the Manage Roles permission in order to execute this command.'
      )
      return msg.delete({ timeout: 10000 })
    }
    if (
      ...
      let msg = await message.channel.send(
        `Proper command usage: ${prefix}rolepicker @[role] emoji @[role] emoji @[role] emoji etc. (maximum 25)`
      )
      return msg.delete({ timeout: 10000 })
    }
    if (!message.member.hasPermission('MANAGE_GUILD')) {
      let msg = await message.chanel.send(
    ...
    roles.forEach(role => {
      rolepicker.addField(
        role.name,
        message.client.emojis.cache.get(emojis[i]) || emojis[i]
      )
