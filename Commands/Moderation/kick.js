  ...
  async execute (message, args, prefix) {
    ...
    if (!message.guild.me.hasPermission('KICK_MEMBERS')) {
      let msg = await message.channel.send(
        'I require the `Kick Members` permission in order to perform this action.'
      )
    ...
    if (!member || !args[1]) {
      let msg = await message.channel.send(
        `Proper command usage: ${prefix}kick @[user] [reason]`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    if (member.id == message.author.id) {
      let msg = await message.channel.send(
        `I mean you could simply leave the server.`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    ...
    if (modhighestrole <= memberhighestrole) {
      let msg = await message.channel.send(
        'Your roles must be higher than the roles of the person you want to kick!'
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    if (
      ...
      let msg = await message.channel.send(
        `It appears that you lack permissions to kick. In case you have them, make sure that my role is higher than the role of the person you want to kick!`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    ...
    const kickembed = new Discord.MessageEmbed()
      ...
      .setTimestamp()
    await member.send(
      `${author} kicked you from ${message.guild.name} for ${reason}.`
    )
