  ...
  async execute (message, args, prefix) {
    ...
    if (!message.guild.me.hasPermission('MANAGE_ROLES')) {
      let msg = await message.channel.send(
        'I require the Manage Roles permission in order to perform this action!'
      )
    ...
    if (!member) {
      let msg = await message.channel.send(
        `Proper command usage: ${prefix}unmute @[user]`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    if (
      ...
      let msg = await message.channel.send(
        `It appears that you lack permissions to unmute.  In case you have them, make sure that my role is higher than the role of the person you want to unmute!`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    ...
    await member.send(
      `${author} unmuted you earlier from ${message.guild.name}.`
    )
    message.react('✔️')
  }
}
