  ...
  async execute (message, prefix) {
    ...
    if (!member) {
      let msg = await message.channel.send(
        `Proper command usage: ${prefix}record @[user]`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    if (
      !message.member.hasPermission('KICK MEMBERS') ||
      !message.guild.member(member).kickable
    ) {
      let msg = await message.channel.send(
        'You need the Kick Members permission in order to run this command. In case you have it, make sure that my role is higher than the role of the person you want to check the record for!'
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
