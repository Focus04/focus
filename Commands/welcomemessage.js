...
module.exports = {
    ...
    async execute(message, args) {
        ...
        for (let i = 0; i < args.length; i++)
            msg = msg + args[i] + ' ';
        if (!args[0])
            message.channel.send(`Proper command usage: ${prefix}welcomemessage [message]. "[member]" will be replaced with a username.`);
        else
            if (!message.member.hasPermission('MANAGE_GUILD'))
                ...
                if (!welcomechannel)
                    ...
                    let welcomemsg = '`' + message.content + '`';
                    let logchname = await logchannels.get(`logchannel_${message.guild.id}`);
                    let log = message.guild.channels.cache.find(logchname);
                    if (!log)
                        message.channel.send(`Welcome message successfully changed to ${welcomemsg}`);
                    else
