...
module.exports = {
  ...
  async execute (message, prefix) {
    ...
    if (!welcome) {
      let msg = await message.channel.send(
        `You first need to set a channel for messages to be sent in. Use ${prefix}setwelcomechannel to setup one.`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    ...
    let log = await message.guild.channels.cache.find(
      channel => channel.name === logchname
    )
    if (!log)
      message.channel.send(
        `Welcome messages are now set to ${'`' + state + '`'}`
      )
