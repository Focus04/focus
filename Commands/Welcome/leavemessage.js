  ...
  async execute (message, args, prefix) {
    if (!args[0]) {
      let msg = await message.channel.send(
        `Proper command usage: ${prefix}leavemessage [message]. Use [user] to be replaced with a username.`
      )
    ...
    let msg = args.join(' ')
    let leavechname = await leavechannels.get(
      `leavechannel_${message.guild.id}`
    )
    let leavechannel = await message.guild.channels.cache.find(
      ch => ch.name === `${leavechname}`
    )
    if (!leavechannel) {
      let msg = await message.channel.send(
        `You need to set a channel for leave messages to be sent in. Use ${prefix}setleavechannel to setup one.`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    ...
    if (!log)
      message.channel.send(
        `Leave message successfully changed to ${'`' + msg + '`'}`
      )
