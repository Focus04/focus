  ...
  async execute (message, args, prefix) {
    if (!args[0]) {
      let msg = await message.channel.send(
        `Proper command usage: ${prefix}welcomemessage [message]. Use [user] to be replaced with a username.`
      )
    ...
    let welcomechname = await welcomechannels.get(
      `welcomechannel_${message.guild.id}`
    )
    let welcomechannel = await message.guild.channels.cache.find(
      ch => ch.name === `${welcomechname}`
    )
    if (!welcomechannel) {
      let msg = await message.channel.send(
        `You need to set a channel for welcome messages to be sent in. Use ${prefix}setwelcomechannel to setup one.`
      )
      msg.delete({ timeout: 10000 })
      return message.react('❌')
    }
    ...
    if (!log)
      message.channel.send(
        `Welcome message successfully changed to ${'`' + msg + '`'}`
      )
